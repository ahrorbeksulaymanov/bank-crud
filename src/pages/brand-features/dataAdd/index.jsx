...
const AddBrandFeatures = () => {
  const [loading, setloading] = useState(false);
  const [checked, setchecked] = useState(true);
  const match = useRouteMatch("/brand/features-add/:id/:featureId");
  ...
  useEffect(() => {
    if (match.params.featureId != 0) {
      ...
      axios({
        url: PATH_API + `/brand/feature-one/${match.params.featureId}`,
        method: "get",
        headers: {
  ...
  const updateData = (val) => {
    ...
    val.brandId = match.params.id;
    const token = localStorage.getItem("token");
    if(match.params.featureId == 0){
      axios({
        url: PATH_API + `/brand/feature`,
        method: "POST",
        data: val,
        headers: {
          Authorization: "Bearer " + token,
      ...
      axios({
        url: PATH_API + `/brand/feature/${match.params.featureId}`,
        method: "PUT",
        data: val,
        headers: {
          Authorization: "Bearer " + token,
      ...
      <div className="bank-add-wrapper">
        <div>
          ...
          >
            ...
            <Form.Item
              label="Hususiyat holati"
              name="active"
              className="my-5"
              rules={[{ required: false, message: "Iltimos Hususiyat holati amal qilish muddatini kiriting!" }]}
            >
...
export default AddBrandFeatures;
